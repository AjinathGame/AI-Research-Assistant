import React from "react";
import {
    Brain, FileText, Search, Database, Sparkles, ShieldCheck, Zap, Users, SearchCheck, Quote, FileQuestion, Lightbulb,
} from "lucide-react";
import Navbar from "../components/Home/Navbar";
import AboutViewImg from "../assets/About_view.png";
import Footer from "../components/Home/Footer";

const About = () => {

    const features = [
        {
            icon: FileText,
            title: "Document Upload",
            text: "Upload research papers, PDFs, and study materials for quick and organized document analysis",
        },
        {
            icon: Zap,
            title: "Fast Processing",
            text: "Process and prepare document content efficiently for faster searching and AI-powered responses",
        },
        {
            icon: Search,
            title: "Semantic Retrieval",
            text: "Find the most relevant information from documents based on the meaning of your query.",
        },
        {
            icon: ShieldCheck,
            title: "Reliable Sources",
            text: "Get grounded answers with relevant document and page-level references for better trust and accuracy.",
        },
        {
            icon: Quote,
            title: "Source Citations",
            text: "View document and page-level references so you can easily verify where the answer comes from",
        },
        {
            icon: FileQuestion,
            title: "AI-Powered Question Answering",
            text: "Ask questions about your uploaded research documents and get contextual answers from the available content.",
        },
        {
            icon: Lightbulb,
            title: "Research Insights",
            text: "Quickly understand important concepts, key points, and relevant information from large research documents.",
        },
        {
            icon: SearchCheck,
            title: "Smart Document Search",
            text: "Search across uploaded documents using natural-language queries instead of exact keywords.",
        },
    ];

    return (

        <div className="min-h-screen w-full overflow-x-hidden bg-[#f8f7ff] text-gray-900">

            <Navbar />

            <main>

                <section
                    className="relative mt-4 min-h-[420px] overflow-hidden bg-cover bg-center bg-no-repeat sm:min-h-[480px] lg:min-h-[520px]"
                    style={{ backgroundImage: `url(${AboutViewImg})` }}>

                    <div className="absolute inset-0 bg-black/35" />

                    <div className="relative z-10 flex min-h-[420px] items-center px-5 py-12 sm:min-h-[480px] sm:px-10 sm:py-16 lg:min-h-[520px] lg:px-20">

                        <div className="w-full max-w-2xl">
                            <h1 className="text-3xl font-bold leading-tight text-white sm:text-5xl lg:text-6xl">
                                About AI-Search Assistant{" "}
                                <span className="text-3xl sm:text-4xl">🤖</span>
                            </h1>

                            <p className="mt-5 max-w-xl text-sm leading-6 text-white/90 sm:mt-6 sm:text-lg sm:leading-7">
                                An intelligent search platform designed to help students and
                                researchers upload, organize, search, and understand their
                                research documents using Artificial Intelligence.
                            </p>
                        </div>

                    </div>

                </section>

                <section className="px-5 py-12 sm:px-8 sm:py-16 lg:px-16 xl:px-20">

                    <div className="mx-auto grid max-w-6xl items-center gap-8 sm:gap-10 lg:grid-cols-2 lg:gap-12">

                        <div>
                            <p className="mb-3 text-xs font-bold uppercase tracking-widest text-purple-600 sm:text-sm">
                                Our Project
                            </p>

                            <h2 className="text-2xl font-bold leading-tight text-gray-900 sm:text-4xl">
                                Making Search Faster and Smarter
                            </h2>

                            <p className="mt-4 text-sm leading-6 text-gray-600 sm:mt-5 sm:text-base sm:leading-7">
                                AI Search Assistant is a web-based system that helps users
                                work with research papers, notes, and other study materials.
                                Instead of manually searching through lengthy documents, users
                                can upload their documents and use an AI-powered interface to
                                work with their research content.
                            </p>

                            <p className="mt-4 text-sm leading-6 text-gray-600 sm:text-base sm:leading-7">
                                The system uses document processing, text chunking, embeddings,
                                and vector database technology to prepare research documents for
                                intelligent retrieval.
                            </p>
                        </div>

                        <div className="w-full rounded-2xl border border-purple-100 bg-white p-5 shadow-lg sm:rounded-3xl sm:p-8">

                            <div className="grid grid-cols-1 gap-4 min-[450px]:grid-cols-2 sm:gap-5">

                                <div className="rounded-2xl bg-purple-50 p-4 sm:p-5">
                                    <FileText size={26} className="text-purple-600 sm:size-7" />
                                    <h3 className="mt-3 font-semibold sm:mt-4">
                                        PDF Processing
                                    </h3>
                                    <p className="mt-2 text-xs leading-5 text-gray-500 sm:text-sm">
                                        Process and organize research documents.
                                    </p>
                                </div>

                                <div className="rounded-2xl bg-indigo-50 p-4 sm:p-5">
                                    <Search size={26} className="text-indigo-600 sm:size-7" />
                                    <h3 className="mt-3 font-semibold sm:mt-4">Smart Search</h3>
                                    <p className="mt-2 text-xs leading-5 text-gray-500 sm:text-sm">
                                        Find relevant information efficiently.
                                    </p>
                                </div>

                                <div className="rounded-2xl bg-violet-50 p-4 sm:p-5">
                                    <Database size={26} className="text-violet-600 sm:size-7" />
                                    <h3 className="mt-3 font-semibold sm:mt-4">
                                        Vector Database
                                    </h3>
                                    <p className="mt-2 text-xs leading-5 text-gray-500 sm:text-sm">
                                        Store document embeddings for retrieval.
                                    </p>
                                </div>

                                <div className="rounded-2xl bg-blue-50 p-4 sm:p-5">
                                    <Sparkles size={26} className="text-blue-600 sm:size-7" />
                                    <h3 className="mt-3 font-semibold sm:mt-4">AI Assistance</h3>
                                    <p className="mt-2 text-xs leading-5 text-gray-500 sm:text-sm">
                                        Support intelligent research workflows.
                                    </p>
                                </div>

                            </div>

                        </div>

                    </div>

                </section>

                <section className="bg-white px-5 py-12 sm:px-8 sm:py-16 lg:px-16 xl:px-20">

                    <div className="mx-auto max-w-6xl text-center">

                        <p className="text-xs font-bold uppercase tracking-widest text-purple-600 sm:text-sm">
                            Key Features
                        </p>

                        <h2 className="mt-2 text-2xl font-bold sm:mt-3 sm:text-4xl">
                            What Our System Provides
                        </h2>

                        <div className="mt-8 grid gap-5 min-[500px]:grid-cols-2 sm:mt-12 lg:grid-cols-4">

                            {features.map((item, index) => {
                                const Icon = item.icon;

                                return (
                                    <div
                                        key={index}
                                        className="rounded-2xl border-2 border-b-indigo-600 border-l-0 border-r-0 border-t-0 bg-[#faf9ff] p-5 text-left shadow-lg transition duration-300 hover:-translate-y-1 hover:shadow-xl sm:p-6"
                                    >
                                        <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-purple-100 sm:h-12 sm:w-12">
                                            <Icon size={22} className="text-purple-600 sm:size-6" />
                                        </div>

                                        <h3 className="mt-4 text-base font-bold sm:mt-5 sm:text-lg">
                                            {item.title}
                                        </h3>

                                        <p className="mt-2 text-xs leading-5 text-gray-500 sm:text-sm sm:leading-6">
                                            {item.text}
                                        </p>
                                    </div>

                                );

                            })}

                        </div>

                    </div>

                </section>

                <section className="px-5 py-12 sm:px-8 sm:py-16 lg:px-16 xl:px-20">

                    <div className="mx-auto max-w-6xl rounded-2xl bg-gradient-to-r from-[#3b25cc] to-[#5637f5] p-6 sm:rounded-3xl sm:p-10 lg:p-12">

                        <div className="grid items-center gap-8 lg:grid-cols-2 lg:gap-10">
                            <div>
                                <p className="text-xs font-semibold uppercase tracking-widest text-[#f4f4f5] sm:text-sm">
                                    Our Goal
                                </p>

                                <h2 className="mt-2 text-2xl font-bold leading-tight text-white sm:mt-3 sm:text-4xl">
                                    Simplify the Research Experience
                                </h2>

                                <p className="mt-4 text-sm leading-6 text-gray-200 sm:mt-5 sm:text-base sm:leading-7">
                                    Our goal is to reduce the time and effort required to search
                                    through large amounts of research material and provide users
                                    with an intelligent platform for managing and understanding
                                    their documents.
                                </p>
                            </div>

                            <div className="grid gap-4 min-[450px]:grid-cols-2">
                                <div className="rounded-xl bg-white/10 p-4 sm:rounded-2xl sm:p-5">
                                    <Users size={24} className="text-[#c9b6ff]" />

                                    <h3 className="mt-3 font-semibold text-white">
                                        For Students
                                    </h3>

                                    <p className="mt-2 text-xs leading-5 text-white/80 sm:text-sm">
                                        Useful for notes, papers, assignments, and academic study.
                                    </p>
                                </div>

                                <div className="rounded-xl bg-white/10 p-4 sm:rounded-2xl sm:p-5">
                                    <Brain size={24} className="text-[#c9b6ff]" />

                                    <h3 className="mt-3 font-semibold text-white">
                                        For Researchers
                                    </h3>

                                    <p className="mt-2 text-xs leading-5 text-white/80 sm:text-sm">
                                        Helps organize and explore research documents efficiently.
                                    </p>
                                </div>

                            </div>

                        </div>

                    </div>

                </section>

                <section className="border-t border-gray-200 bg-white px-5 py-8 text-center sm:px-8 sm:py-10">
                    <h2 className="text-xl font-bold sm:text-2xl">
                        AI Search Assistant System
                    </h2>

                    <p className="mx-auto mt-2 max-w-2xl text-xs leading-5 text-gray-500 sm:mt-3 sm:text-sm sm:leading-6">
                        An AI-powered platform for intelligent research document processing,
                        semantic search, and efficient knowledge retrieval.
                    </p>
                </section>

            </main>

            <Footer />

        </div>

    );
};

export default About;